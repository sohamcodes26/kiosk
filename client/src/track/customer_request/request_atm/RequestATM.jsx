...
const RequestATM = () => {
  ...
  const handleSubmit = () => {
    if (!selectedNetwork || !selectedType || !reason) {
      alert("Please select card network, type, and reason.");
      return;
    }
    
    console.log(`Requesting ${selectedType} ${selectedNetwork} ATM card. Reason: ${reason}`);
    
    // Pass the request-atm type to the dynamic end session component
    navigate('/end-session', { state: { type: 'request-atm' } });
  };
    ...
    <div className="flex flex-col h-screen w-full font-sans bg-[#e9eff6]">
      ...
      <main className="flex-grow flex flex-col items-center p-8 relative">
        <div className="w-full max-w-3xl flex items-center justify-center relative mb-8 mt-2">
          ...
          <h2 className="text-[34px] font-semibold text-black">{t.requestAtmTitle}</h2>
        </div>
        ...
        <div className="bg-white w-full max-w-3xl rounded-lg shadow-sm border border-gray-200 p-10 flex flex-col gap-8">
          ...
          {/* Card Network */}
          <div>
            <label className={labelStyle}>{t.cardNetwork}</label>
            <div className="grid grid-cols-3 gap-4">
              <button onClick={() => setSelectedNetwork('RuPay')} className={getBtnStyle(selectedNetwork === 'RuPay')}>{t.rupay}</button>
              <button onClick={() => setSelectedNetwork('Mastercard')} className={getBtnStyle(selectedNetwork === 'Mastercard')}>{t.mastercard}</button>
              <button onClick={() => setSelectedNetwork('Visa')} className={getBtnStyle(selectedNetwork === 'Visa')}>{t.visa}</button>
            </div>
          </div>

          {/* Card Type */}
          <div>
            <label className={labelStyle}>{t.cardType}</label>
            <div className="grid grid-cols-2 gap-4">
              <button onClick={() => setSelectedType('Domestic')} className={getBtnStyle(selectedType === 'Domestic')}>{t.domestic}</button>
              <button onClick={() => setSelectedType('International')} className={getBtnStyle(selectedType === 'International')}>{t.international}</button>
            </div>
          </div>
