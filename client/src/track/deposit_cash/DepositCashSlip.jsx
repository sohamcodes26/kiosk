...
import './DepositCashSlip.css';

const DepositCashSlip = ({
  branch = '',
  date = '',
  accountNumber = '',
  accountHolderName = '',
  ...
  const accChars = (accountNumber || '').padEnd(14, ' ').split('').slice(0, 14); // 13 or 14 usually, deposit has 13 inputs
  const panChars = (panNumber || '').padEnd(10, ' ').split('').slice(0, 10);

  const getDenomTotal = (count, multiplier) => {
    const val = parseInt(count) || 0;
    return val > 0 ? val * multiplier : '';
  };

  const getDenomPs = (count) => {
    const val = parseInt(count) || 0;
    return val > 0 ? '00' : '';
  };
      ...
      <div className="deposit-slip">
        ...
        <div className="panel p1">
          ...
          <div className="p1-body">
            ...
            <table className="chtable">
              ...
              <tbody>
                <tr><td>Cash Deposit</td><td className="fill-blue text-center font-bold">{amount}</td><td className="fill-blue text-center font-bold">00</td></tr>
        ...
        <div className="panel p2">
          ...
          <table className="ntable">
            ...
            <tbody>
              <tr><td>2000 x</td><td className="fill-blue text-center">{notes2000 || ''}</td><td className="fill-blue text-center">{getDenomTotal(notes2000, 2000)}</td><td className="text-center fill-blue">{getDenomPs(notes2000)}</td></tr>
              <tr><td>500 x</td><td className="fill-blue text-center">{notes500 || ''}</td><td className="fill-blue text-center">{getDenomTotal(notes500, 500)}</td><td className="text-center fill-blue">{getDenomPs(notes500)}</td></tr>
              <tr><td>200 x</td><td className="fill-blue text-center">{notes200 || ''}</td><td className="fill-blue text-center">{getDenomTotal(notes200, 200)}</td><td className="text-center fill-blue">{getDenomPs(notes200)}</td></tr>
              <tr><td>100 x</td><td className="fill-blue text-center">{notes100 || ''}</td><td className="fill-blue text-center">{getDenomTotal(notes100, 100)}</td><td className="text-center fill-blue">{getDenomPs(notes100)}</td></tr>
              <tr><td>50 x</td><td className="fill-blue text-center">{notes50 || ''}</td><td className="fill-blue text-center">{getDenomTotal(notes50, 50)}</td><td className="text-center fill-blue">{getDenomPs(notes50)}</td></tr>
              <tr><td>20 x</td><td className="fill-blue text-center">{notes20 || ''}</td><td className="fill-blue text-center">{getDenomTotal(notes20, 20)}</td><td className="text-center fill-blue">{getDenomPs(notes20)}</td></tr>
              <tr><td>10 x</td><td className="fill-blue text-center">{notes10 || ''}</td><td className="fill-blue text-center">{getDenomTotal(notes10, 10)}</td><td className="text-center fill-blue">{getDenomPs(notes10)}</td></tr>
              <tr><td>5 x</td><td className="fill-blue text-center">{notes5 || ''}</td><td className="fill-blue text-center">{getDenomTotal(notes5, 5)}</td><td className="text-center fill-blue">{getDenomPs(notes5)}</td></tr>
              <tr><td>नाणी/Coins</td><td className="fill-blue text-center">{coins || ''}</td><td className="fill-blue text-center">{parseInt(coins) > 0 ? coins : ''}</td><td className="text-center fill-blue">{getDenomPs(coins)}</td></tr>
              <tr className="trow">
                <td colSpan="2" style={{ textAlign: 'right' }}>एकुण/Total</td>
                <td className="fill-blue text-center font-bold">{amount}</td>
                <td className="text-center font-bold fill-blue">00</td>
              </tr>
            </tbody>
          </table>
...
export default DepositCashSlip;
